...
module.exports = function (app,app_data) {
  ...
  async  (req, res) => {
    try {
      ...
      for (let i = 0; i < users.length; i++){
        if (users[i].email === email_ && users[i].username === username_ || users[i].email === email_ || users[i].username === username_){
  ...
  app.post("/read-user", async (req, res) => {
    try {
      ...
      const username = await userModel.findOne({username: req.body.userlogin}).lean();

      bcrypt.hash(req.body.passlogin, saltRounds, function(err,hash){
        current_hashed_password = hash;

        const userInfo = userModel.findOne({username: req.body.userlogin, password: current_hashed_password}).lean();

        if (userInfo){
          loginInfo = {username: req.body.userlogin, password: req.body.passlogin};
          res.redirect(`/`);
        } else {
          loginInfo = null;
          res.redirect(`/`);
